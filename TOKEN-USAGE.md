...
RepoMux keeps repository work out of the coordinator's conversation, so the coordinator does not spend tokens reading every agent's detailed output.
When RepoMux assigns work, it starts a separate ephemeral `codex exec` session for each repository.
...
RepoMux adds together the usage reported by every attempt and saves the total under `execution.usage` in the repository result.
